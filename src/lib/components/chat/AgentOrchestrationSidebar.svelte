<script lang="ts">
	...
	let activeSessionIds: Set<string> = new Set();
	let globalStatus: 'idle' | 'active' | 'done' = 'idle';
	let statusMessage = 'Waiting for a request…';
	let viewMode: 'timeline' | 'flow' = 'timeline';

	// Reset local state when orchestration is reset (new message sent)
	$: if (!$orchestrationDone) {
		timelineItems = [{ ...ORCHESTRATOR_IDLE }];
		activeSessionIds = new Set();
		globalStatus = 'idle';
		statusMessage = 'Waiting for a request…';
	}
	...
	function handleMessage(msg: OrchestrationMessage) {
		...
		// Ignore events from unknown sessions (unless it's a new session_start)
		if (sid && !activeSessionIds.has(sid) && msg.step !== 'session_start') {
			return;
		}
		...
		switch (msg.step) {
			...
			case 'routing':
				updateItem('orchestrator', { action: 'Selecting the best agent…', status: 'active' });
				statusMessage = msg.message ?? 'Routing…';
				break;
			...
			case 'agent_active':
				...
				statusMessage = `Routing to ${msg.agent_label ?? msg.agent_id}…`;
				break;
			...
			case 'agent_done':
				...
				statusMessage = `${msg.agent_label ?? msg.agent_id} responded`;
				break;
