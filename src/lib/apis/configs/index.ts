...
export const getOrchestratorConfig = async (token: string) => {
	let error = null;
	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/orchestrator`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const setOrchestratorConfig = async (token: string, config: object) => {
	let error = null;
	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/orchestrator`, {
		method: 'POST',
		headers: {
